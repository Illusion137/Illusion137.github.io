...
function DitheredWaves({ waveSpeed, waveFrequency, waveAmplitude, waveColor, colorNum, pixelSize, disableAnimation, enableMouseInteraction, mouseRadius }: DitheredWavesProps) {
	...
	useFrame(({ clock }) => {
		...
		const dpr = gl.getPixelRatio();
		u.resolution.value.set(Math.floor(size.width * dpr), Math.floor(size.height * dpr));
...
export default function Dither({
	...
	return (
		<Canvas className="relative h-full w-full" camera={{ position: [0, 0, 6] }} dpr={1} frameloop="always" gl={{ antialias: true, preserveDrawingBuffer: true }}>
			<DitheredWaves
				waveSpeed={waveSpeed}
