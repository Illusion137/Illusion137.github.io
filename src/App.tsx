...
function resolve_from_search(search: string): {
	...
	return { playlist_screen_id, active_playlist, active_track_index };
}

function build_path(playlist_screen_id: string | null, active_playlist: Playlist | null, active_track_index: number): string {
...
function App() {
	...
	const playlist_projects = useMemo<Project[]>(() => projects.filter((project) => active_playlist?.project_ids.includes(project.id)), [active_playlist]);
	...
	useEffect(() => {
		const next_path = build_path(playlist_screen_id, active_playlist, active_track_index);
	...
	}, [playlist_screen_id, active_playlist, active_track_index]);
	...
	return (
		<div className="bg-background min-h-screen w-full">
			...
			{playlist_screen_id === null ? (
				<LandingScreen face_src={'profile_cover.jpg'} playlists={playlists} projects={projects} on_select_playlist={on_select_playlist} on_select_project={on_select_project} />
			) : (
			...
			<div className="fixed inset-x-0 bottom-0 z-10">
				<BottomBar
					...
					progress={1 / playlist_projects.length}
					socials={socials}
